...
switch (_code) do {
	...
	if(_shift) then {_handled = true;};
		...
		if !(license_civ_rebel || license_civ_pmcSoldier) exitWith { hintSilent "You need a rebel/pmc license!"; };
	...
	/*
		if(_shift) then {_handled = true;};
		if(_shift && playerSide == civ && {!isNull cursorTarget} && {cursorTarget isKindOf "Man"} && {(isPlayer cursorTarget)} && {(side cursorTarget in [civilian,independent,west])} && {alive cursorTarget} && {cursorTarget distance player < 3.5} && {!(cursorTarget GVAR "Escorting")} && {!(cursorTarget GVAR "restrained")} && {speed cursorTarget < 1}) then {
			hint "Exacuted zip tie";
